...
@st.cache_data(ttl=900)
def fetch_ohlcv_timeframe(symbol: str, interval: str = "1d", period: str = None) -> pd.DataFrame:
    """
    ...
    while retries <= max_retries:
        try:
            # Download quotes with raw parameters
            df = yf.download(
                tickers=formatted_symbol, 
                period=period, 
                interval=interval, 
            ...
            if df is not None and not df.empty:
                ...
                df_clean = df[required_cols].copy()
                df_clean = df_clean.dropna(subset=['Close'])
                # Only drop zero volume on standard daily or higher candles
